...
do
  ...
  OP_TEAM=$($PSQL "SELECT name FROM teams WHERE name='$OPPONENT'")
  ...
  then
    if [[ $YEAR != "year" ]]
    then
      INSERT_GAMES=$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES('$YEAR','$ROUND','$W_ID','$O_ID','$WINNER_GOALS','$OPPONENT_GOALS')")
      if [[ $INSERT_OP_TEAM == "INSERT 0 1" ]] 
      then
